 ...
 * Return { url } kalau sukses, atau { error } kalau gagal.
 */
async function uploadFotoProfil(file, userId) {
  ...
  const { data } = supabaseClient.storage.from('avatars').getPublicUrl(namaFile);
  return { url: data.publicUrl };
}

/** Ambil path file di dalam bucket dari URL publik Supabase Storage */
function ambilPathDariUrlPublik(url, bucketName) {
  if (!url) return null;
  const penanda = `/public/${bucketName}/`;
  const idx = url.indexOf(penanda);
  if (idx === -1) return null;
  return url.substring(idx + penanda.length);
}
